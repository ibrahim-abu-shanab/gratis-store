...
const upload = multer({
  ...
  // الحد الأقصى لحجم الملف = 10 ميغابايت (10 × 1024 × 1024 = 10MB)
});
...
app.get("/api/products", (req, res) => {
  ...
  // فلتر حسب الفئة الرئيسية
  if (req.query.category) {
    products = products.filter(p => p.category === req.query.category);
  }

  // فلتر حسب الفئة الفرعية (للتاكي)
  if (req.query.subcategory) {
    products = products.filter(p => p.subcategory === req.query.subcategory);
  }

  // منتجات عشوائية للصفحة الرئيسية
  if (req.query.random) {
...
app.patch("/api/cart/:id", (req, res) => {
  ...
  const product = db.products.find(p => p.id === item.product_id);
  if (product) {
    product.stock += item.quantity - newQty;
  }
